...
NAN_METHOD(EffectsNative::RollImage) {
    EffectsNative *kEffectNative = Nan::ObjectWrap::Unwrap<EffectsNative>(info.This()); 

    if (info.Length() == 1) {
        if (info[0]->IsString()) {
            try {
                kEffectNative->image.roll(Magick::Geometry(std::string(*Nan::Utf8String(info[0]))));
            ...
            if (valueValue->IsString()) {
                try {
                    kEffectNative->image.roll(Magick::Geometry(*Nan::Utf8String(valueValue)));
                } catch (Magick::Exception& err) {
                    Nan::ThrowReferenceError(Nan::New<v8::String>(err.what()).ToLocalChecked());
                } catch (...) {
                    Nan::ThrowReferenceError(Nan::New<v8::String>("Unknown error").ToLocalChecked());
            ...
            try {
                kEffectNative->image.roll(Magick::Geometry(
