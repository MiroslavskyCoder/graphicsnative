...
NAN_METHOD(EffectsNative::ThumbnailImage) {
    EffectsNative *kEffectNative = Nan::ObjectWrap::Unwrap<EffectsNative>(info.This());
    SizesOptions sizes(0, 0);

    if (info.Length() == 1) {
        if (info[0]->IsString()) {
            Nan::Utf8String stdSizeGeometry(info[0]); 
            try { 
                kEffectNative->image.thumbnail(Magick::Geometry(std::string(*stdSizeGeometry)));
            ...
            if (valueValue->IsString()) {
                kEffectNative->image.thumbnail(Magick::Geometry(*Nan::Utf8String(valueValue)));
                return;
            }
            ...
            try { 
                kEffectNative->image.thumbnail(Magick::Geometry(
