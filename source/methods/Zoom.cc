...
NAN_METHOD(EffectsNative::ZoomImage) {
    ...
    if (info.Length() == 1) {
        if (info[0]->IsString()) {
            ...
            if (valueValue->IsString()) {
                try { 
                    kEffectNative->image.zoom(Magick::Geometry(*Nan::Utf8String(valueValue)));
                } catch (Magick::Exception& err) {
                    Nan::ThrowReferenceError(Nan::New<v8::String>(err.what()).ToLocalChecked());
                } catch (...) {
                    Nan::ThrowReferenceError(Nan::New<v8::String>("Unknown error").ToLocalChecked());
            ...
            try { 
                kEffectNative->image.zoom(Magick::Geometry(
